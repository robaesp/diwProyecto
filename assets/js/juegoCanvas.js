...
class Juego {
	constructor() {
		...
		this.siguienteDireccion = this.direcciones.KEY_RIGHT;
		this.direccion = this.direcciones.KEY_RIGHT;
		let dis = this;
		...
		$("#canvas").click(evento => {
			...
			if (x <= tercioXCanvas && y >= tercioYCanvas && y <= tercioYCanvas * 2) {
				if (
					dis.direccion === dis.direcciones.KEY_UP ||
					dis.direccion === dis.direcciones.KEY_DOWN
				)
					dis.siguienteDireccion = dis.direcciones.KEY_LEFT;
			} else if (
				x >= tercioXCanvas * 2 &&
				x <= tercioXCanvas * 3 &&
				y >= tercioYCanvas &&
				...
				if (
					...
					dis.siguienteDireccion = dis.direcciones.KEY_RIGHT;
			} else if (
				x >= tercioXCanvas &&
				x <= tercioXCanvas * 2 &&
				y >= 0 &&
				...
				if (
					dis.direccion === dis.direcciones.KEY_RIGHT ||
					dis.direccion === dis.direcciones.KEY_LEFT
				)
					dis.siguienteDireccion = dis.direcciones.KEY_UP;
			} else if (
				x >= tercioXCanvas &&
				x <= tercioXCanvas * 2 &&
				y >= tercioYCanvas * 2 &&
				...
				if (
					...
					dis.siguienteDireccion = dis.direcciones.KEY_DOWN;
			}
		});
		...
		$(document).keydown(evento => {
			...
			if (direccion) {
				if (
					(this.direccion === this.direcciones.KEY_RIGHT ||
						this.direccion === this.direcciones.KEY_LEFT) &&
					(direccion === "KEY_UP" || direccion === "KEY_DOWN")
				)
					this.siguienteDireccion = this.direcciones[direccion];
				else if (
					(this.direccion === this.direcciones.KEY_UP ||
						this.direccion === this.direcciones.KEY_DOWN) &&
					(direccion === "KEY_RIGHT" || direccion === "KEY_LEFT")
				)
					this.siguienteDireccion = this.direcciones[direccion];
			}
		});
	...
	dibujarSerpiente() {
		...
		switch (this.direccion) {
			case this.direcciones.KEY_RIGHT:
				this.serpiente[0].x++;
				break;
			case this.direcciones.KEY_LEFT:
				this.serpiente[0].x--;
				break;
			case this.direcciones.KEY_UP:
				this.serpiente[0].y--;
				break;
			case this.direcciones.KEY_DOWN:
				this.serpiente[0].y++;
				break;
		}
